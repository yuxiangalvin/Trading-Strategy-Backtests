# -*- coding: utf-8 -*-
"""
Created on Tue Jul 30 14:06:22 2019
...
column_list = [['Date','Max Profit', 'Above Ratio', 'Below Ratio','Real Time Theo Change', 'Abs Real Time Theo Change', 'Max min Spread','Average Theo', 'Std of Theo', 'Exit Reason', 'Base Start', 'Base End', 'Test Start', 'Test End', 'SMA Side Ratio Threshold', 'Bounce Stop Threshold', 'Over Limit Time', 'Trading Cost', 'Test time start price', 'Test time end price', 'Test Price Move', 'Abs Test Price Move', 'ES_base_start_price', 'ES_base_end_price', 'ES_base_price_change', 'ES_test_start_price', 'ES_test_end_price', 'ES_test_price_change', 'ES_test_price_abs_change', 'RTY_base_start_price', 'RTY_base_end_price', 'RTY_base_price_change', 'RTY_test_start_price', 'RTY_test_end_price', 'RTY_test_price_change', 'RTY_test_price_abs_change']]
for test_trade_day in trade_day_list:
    ...
    for base_time_end in base_time_end_list:
        ...
        for test_length in test_length_list:
            ...
            ES_base_timeframe_df = ts.select_date_timeframe(ES_Tele.df, test_trade_day, base_time_start, base_time_end)
            ES_test_timeframe_df = ts.select_date_timeframe(ES_Tele.df, test_trade_day, test_time_start, test_time_end)
            RTY_base_timeframe_df = ts.select_date_timeframe(RTY_Tele.df, test_trade_day, base_time_start, base_time_end)
            RTY_test_timeframe_df = ts.select_date_timeframe(RTY_Tele.df, test_trade_day, test_time_start, test_time_end)
            ...
            try:
                ...
                ES_base_start_price = 0
                ES_base_end_price = 0
                ES_base_price_change = ES_base_end_price - ES_base_start_price
                ES_test_start_price = 0
                ES_test_end_price = 0
                ES_test_price_change = ES_test_end_price - ES_test_start_price
                ES_test_price_abs_change = np.absolute(ES_test_price_change)
            
            try:
                RTY_base_start_price = RTY_base_timeframe_df['TradePrice'][0]
                RTY_base_end_price = RTY_base_timeframe_df['TradePrice'][-1]
                RTY_base_price_change = RTY_base_end_price - RTY_base_start_price
                RTY_test_start_price = RTY_test_timeframe_df['TradePrice'][0]
                RTY_test_end_price = RTY_test_timeframe_df['TradePrice'][-1]
                RTY_test_price_change = RTY_test_end_price - RTY_test_start_price
                RTY_test_price_abs_change = np.absolute(RTY_test_price_change)
            except:
                RTY_base_start_price = 0
                RTY_base_end_price = 0
                RTY_base_price_change = RTY_base_end_price - RTY_base_start_price
                RTY_test_start_price = 0
                RTY_test_end_price = 0
                RTY_test_price_change = RTY_test_end_price - RTY_test_start_price
                RTY_test_price_abs_change = np.absolute(RTY_test_price_change)
...
final_result_df.to_excel("C:\\Users\\achen\\Desktop\\Monocular\\Automatic Backtest\\RTY Delta Side\\RTY_20170101_20171231_ALL_FEATURES_with_ESRTY.xlsx")
