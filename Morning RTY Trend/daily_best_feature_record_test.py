# -*- coding: utf-8 -*-
"""
Created on Wed Jul 24 11:58:03 2019
...
import datetime
def daily_best_feature_record_test(Tele2, trade_day_list, base_time_start, base_time_end, test_time_start, test_time_end, sma_threshold, stop_threshold, bounce_stop_threshold, over_limit_length, loss_stop, trade_cost):
...
#    check_list = []
    ...
    for trade_day in trade_day_list:
        ...
        test_start_price = test_timeframe_df['TheoPrice'][0]
        test_end_price = test_timeframe_df['TheoPrice'][-1]
        test_price_move = test_end_price - test_start_price
        abs_test_price_move = np.absolute(test_price_move)
...
#            check_list.append([sell_price, buy_price, close_time])
        ...
        if (profit < 0):
            ...
            loss_trade_day_info.append(day_info)
            
            
            
    time_stop_count = trade_count - limit_stop_count - bounce_stop_count
    return np.array(daily_profit_list), trade_count, limit_stop_count, bounce_stop_count, time_stop_count, loss_trade_day_info, above_ratio, below_ratio, theo_change, absolute_theo_change, max_min_spread, average, std_dev, exit_reason, test_start_price, test_end_price, test_price_move, abs_test_price_move
